...
### 拆卸取景鏡頭
如果萬一在調整好對焦螺絲後，從底片室對一目標物正確對焦後，發現從取景器裡看進去沒有合焦，那麼這時候就需要來調整取景鏡頭了。取景鏡頭的固定方式是將鏡頭同時旋入前面板以及一塊固定板，再透過兩支螺絲將固定板鎖在前面板上，所以首先要做的就是拆掉那兩支螺絲（請參閱「拆卸前面板」章節中照片裡的黃圈就是了），接著轉鬆那片固定板之後就可以將鏡頭轉下來了，不過如果是要微調的話不建議把它整個拆下來：
